...
interface ManagerDriverColumnProps {
  onReassign: (driver: AssignedDriver) => void;
  onCancel: (driver: AssignedDriver) => void;
}

export const createManagerDriversColumns = ({
  onReassign,
  onCancel,
}: ManagerDriverColumnProps): ColumnDef<AssignedDriver>[] => [
  {
    accessorKey: "driver",
    header: "Driver",
    cell: ({ row }) => {
  ...
  },
  {
    id: "lastActive",
    header: "Last Active",
    cell: () => <div className="text-sm">5 months ago</div>, // Mock data
  },
  {
    id: "actions",
    header: "Actions",
      ...
      return (
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3 py-1.5 text-sm font-medium text-white bg-green-500 rounded-md transition-colors">
            Reassign
          </button>
          <button
            onClick={(e) => {
